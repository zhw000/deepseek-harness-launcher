...
- **版本兼容检查**：按插件 `peerDependencies` 里声明的范围，对照当前 dsh 真正自带的包逐一核对，直接写明「需要 dsh ≥ 0.1.7-rc.1，当前是 0.1.5-rc.3」，并指出哪个已发布的 dsh 能让全部插件满足要求。安装前、切换 dsh 版本前、体检里都会检查，不必等插件在浏览器里报错。
...
**安装插件为什么有时要半分钟？** 普通插件几秒就装完（实测一个 60 条依赖的配置，装、卸一个插件都在 2 秒内；pnpm 11 的供应链校验本身不到 1 秒，而且按锁文件内容缓存）。慢的通常是依赖要跑构建脚本——编译原生模块，或像 `cloudflared` 那样从 GitHub 下载二进制——或者下载源超时。连续安装会合并成一次运行，任务面板显示当前阶段；频繁出现 ETIMEDOUT 就去设置里测速换源。卸载在后台进行：点下去插件立即停用，界面不用等 pnpm。

**提示“pnpm 拒绝安装发布不满 24 小时的版本”？** pnpm 11 默认不装刚发布不到 24 小时的版本（`minimumReleaseAge`），用来挡住“刚发布就被投毒”的包；它自己解析到这类版本时会自动记进 `minimumReleaseAgeExclude` 放行。但每次运行它还会重新核对整份锁文件，如果锁文件里有别的工具（例如终端里另一个版本的 pnpm）写进去、没被记录的新版本，连更新别的插件、卸载都会被拦。启动器会按 pnpm 的默认策略把这些确切版本记下来并自动重试，任务日志里有明细；如果你在配置里开了 `minimumReleaseAgeStrict`，则改为在插件页列出这些版本，由你选择等过保护期还是「信任并重试」。
