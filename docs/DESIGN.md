...
体检把排查过的真实故障做成固定检查项，每项给出状态、说明和能修它的页面：运行时与 pnpm 能否运行、dsh 能否输出版本、子进程环境里有没有**空值代理变量**（0.1.3 修过的 `Invalid URL` 根因）、下载源延迟、DSH_HOME 与工作区可写、端口、启动配置是否为 Web 形态、有没有待决的构建脚本、插件是否缺文件或声明不兼容。pnpm 这一项用插件安装时**完全相同的环境**执行 `pnpm config get registry`，所以任何让安装失败的环境问题都会先在这里暴露。
...
- `npm test`：74 个单元测试，覆盖版本选择与剪枝、通道比较、兼容性判定、spec 分类、配置锁（含抢占死锁）、pnpm 构建批准的 YAML 读写、任务队列、shim 生成、ANSI 与日志切分、进程监督（用一个模拟 dsh 的脚本验证就绪、token 掩码、优雅退出与崩溃归因）。
